...
import { getMessages, sendMessage as sendMessageAPI, reactToMessage } from '../../services/api';
import { getSocket } from '../../services/socket';
import { Send, Menu, Info, MessageCircle, Hash, Search, X, Smile } from 'lucide-react';
...
const ChatArea = ({ group, user, onToggleSidebar, onToggleGroupInfo, onlineUsers }) => {
  ...
  const hasUserReacted = (reactions = [], emoji) => {
    return reactions.some(r => r.emoji === emoji && r.userId?.toString() === user._id?.toString());
  };
      ...
      <div className="chat-area">
        <div className="chat-empty">
          ...
          <p className="chat-empty-text">
            Select a group from the sidebar to start chatting, or join a new group to begin a conversation.
          </p>
        </div>
      </div>
    );
    ...
    <div className="chat-area">
      ...
      <div className="chat-header">
        <div className="chat-header-left">
          ...
          <div className="chat-header-avatar" style={{ background: getGroupColor(group.name) }}>
            <Hash size={18} />
          </div>
          <div className="chat-header-info">
            ...
            <div className="chat-header-meta">
              ...
              {onlineUsers?.length || 0} online · {group.members?.length} member{group.members?.length !== 1 ? 's' : ''}
            </div>
          </div>
        </div>
        <div className="chat-header-actions">
          ...
          <button className="chat-header-btn" onClick={onToggleGroupInfo} title="Group Info">
            <Info size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="chat-messages">
        {loading ? (
          ...
          <div className="chat-empty">
            ...
            <p className="chat-empty-text">Be the first to send a message in {group.name}!</p>
          </div>
        ) : (
